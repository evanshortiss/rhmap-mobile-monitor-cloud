      ...
      jobs.forEach(function (job) {
        ...
        job.on('tick-complete', function (err, res, time) {
          if (err) {
            log.error(
              err,
              'Error returned from running job %s after %sms',
              job.meta.name,
              time
            );
          } else {
            log.info('Job %s tick complete in %dms!', job.meta.name, time);
          }
        });
        ...
        job.on('overlapping-call', function () {
          log.info(
            'Job %s attempting to run before a previous tick is complete!',
            job.meta.name
          );
        });
